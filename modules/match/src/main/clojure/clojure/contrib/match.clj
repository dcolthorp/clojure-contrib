(ns clojure.contrib.match
  (:use clojure.contrib.match.compile))
...
(defn- cond-form-for-fn
  ...
  (if (varargs? form)
    (let [n (min-varargs form)
          ...
          all-matches (into []  (concat first-matches
                                        [rest-pattern `(list ~rest-argnames)]))]
...
(defmacro fn-match
  "Works like clojure.core/fn, but argument lists are patterns. Any failed match raises IllegalArgumentException. Does not support pre- and post-conditions.
Example:
  (fn-match this
          ([0] 1)
          ([1] 1)
          ([n] (+ (this (- n 1)) (this (- n 2)))))"
  [& forms]
  (let [[name-container, forms] (if (symbol? (first forms))
                                  [(list (first forms)) (next forms)]
                                  [(list) forms])
...
(defmacro defn-match
  "Works like clojure.core/defn, but argument lists are patterns. Any failed match raises IllegalArgumentException. Does not support pre- and post-conditions."
  [name & forms]
  (let [m (or (meta name) {})
        [m, forms] (if (string? (first forms))
                     [(conj m {:doc (first forms)}), (next forms)]
                     [m, forms])
        [m, forms] (if (map? (first forms))
                     [(conj m (first forms)), (next forms)]
                     [m, forms])
        [m, forms] (if (map? (last forms))
                     [(conj m (last forms)) (butlast forms)]
                     [m forms])]
