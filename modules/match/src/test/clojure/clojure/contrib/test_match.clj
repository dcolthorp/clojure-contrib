(ns clojure.contrib.test-match
  (:use clojure.test
	clojure.contrib.match))

(import '(java.util ArrayList TreeMap))

(deftest test-if-match
  (testing "basic data types use equality checking"
  ...
  (testing "regular expressions match strings"
    (is (not (if-match [#"world" nil] true)))
    (is (not (if-match [#"world" {}] true)))
  ...
  (testing "predicate-like names are used as a test"
...
(deftest test-variable-binding
  (is (= 1 (if-match [a 1] a)))
  (let [a 1]
    (is (if-match [~a 1] true))
    (is (not (if-match [~a 2] true)))))
    
(deftest test-sequence-patterns
  (is (= {:a 1, :b 2}
  ...
  (testing "invalid rest patterns"
    (is (thrown? Exception (eval '(if-match [[&] []] true))))
    (is (thrown? Exception (eval '(if-match [[& bad mojo] []] true)))))
...
(deftest test-map-patterns
  ...
  (is (if-match [{(first %) (even? %)} [2]] true))
  (is (not (if-match [{(first %) (even? %)} [1]] true)))

  (testing ".methodName calls methods"
    (is (if-match [{.toString #"hello"} ["hello world"]] true))
    (is (not (if-match [{.toString #"hello"} ["hell, o world"]] true)))
    (is (not (if-match [{.toString _} nil] true)))))

(deftest test-function-call-patterns
  (is (if-match [(odd? %) 1] true))
  (is (not (if-match [(odd? %) 2] true)))
